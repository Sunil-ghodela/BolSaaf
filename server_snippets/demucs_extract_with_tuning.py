...
def run_extract_voice_job(job_id: int, mode: str = "studio") -> None:
    ...
    # Pick up latest feedback-driven mix ratio (throttled).
    ...
    try:
        ...
        if dry_ratio >= 0.002:
            blended = work_dir / "vocals_blended.wav"
            blend_vocals_dry(vocals_path, input_path, blended, dry_ratio)
            shutil.copy2(blended, final_out)
        else:
